...
import { mockQuestions, mockTestFeedback } from '../../data/mockData';
...
export default function TestMode() {
  ...
  const questions = activeFile ? mockQuestions[activeFile.path] || [] : [];
  ...
  const handleSubmit = () => {
    setSubmitted(true);
    setTimeout(() => {
      setFeedback(mockTestFeedback);
      setErrorTags([
        { tag: 'Belady异常', count: 2, sections: ['Belady 异常'] },
        { tag: '页面置换', count: 3, sections: ['页面置换算法'] },
        { tag: 'TLB', count: 1, sections: ['TLB 与缓存'] },
        { tag: 'x86架构', count: 1, sections: ['TLB 与缓存'] },
      ]);
      openRightDrawer();
    }, 1200);
  };
    ...
    <div className="max-w-2xl mx-auto py-10 px-6">
      ...
      <div className="test-card mb-4">
        ...
        {currentQ.type === 'code' && currentQ.code && (
          <pre className="bg-[#0d1117] text-surface-200 p-4 rounded-lg mb-4 text-sm font-mono overflow-x-auto">
            {currentQ.code}
          </pre>
        )}
